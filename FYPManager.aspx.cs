...
public partial class FYPManager : System.Web.UI.Page
{
    string userName = string.Empty;
    TaskManager ts = new TaskManager();
    protected void Page_Load(object sender, EventArgs e)
    {
        Session["MAIN"] = "FYPManager.aspx";
            userName = Session["USR"].ToString().Trim();
            List<string> students = ts.getAllStudent();
    ...
    public void loadFypDetails()
    {
        ...
        try
        {
            ...
            if(final)
            {
                ...
                List<string> fyp = ts.getFinalResult(student);
                TextBox1.Text = fyp[0];
                TextBox2.Text = fyp[1];
                try
                {
                    double s = double.Parse(fyp[0]) + double.Parse(fyp[1]);
    ...
    protected void Button8_Click(object sender, EventArgs e)
    {
        try
        {
            double fyp1 = double.Parse(TextBox1.Text);
            double fyp2 = double.Parse(TextBox2.Text);
            ts.updateFinalResult(DropDownList1.SelectedItem.ToString(), fyp1.ToString(), fyp2.ToString());
            Session["MSG"] = "The Finalized FYP Marks Are Updated Successfully.";
